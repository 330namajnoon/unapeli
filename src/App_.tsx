...
function App() {
	...
	const users = useSelector((state: RootState) => state.app.peerConnectionUsers);
	...
	const createOffer = (userId: string) => {
		peerConnection.createOffer(userId).then((offer) => {
			socketManager.emit("signal", { userId: id, type: "offer", offer }, roomId);
		});
	};
	...
	const createAnswer = (userId: string, offer: RTCSessionDescriptionInit) => {
		peerConnection.createAnswer(userId, offer).then((answer) => {
			socketManager.emit("signal", { userId: id, type: "answer", answer }, roomId);
		});
	};

	const login = (data: { userId: string; type: string; users: string[]; hasBeenDisconnection: boolean }) => {
		if (id && userExists(data.userId) && data.users.includes(id)) {
			console.log("User already exists");
		} else {
			const hasBeenDisconnection =
				peerConnection.getConnection(data.userId)?.connection.connectionState !== "new";
			console.log(hasBeenDisconnection);
			socketManager.emit("signal", { userId: id, users, hasBeenDisconnection, type: "login" }, roomId);
	...
	const offerManager = (data: { userId: string; offer: RTCSessionDescriptionInit }) => {
		if (data.offer && userExists(data.userId)) {
			createAnswer(data.userId, data.offer);
		}
	};

	const answerManager = (data: { userId: string; answer: RTCSessionDescriptionInit }) => {
		if (data.answer && userExists(data.userId)) {
			peerConnection.setRemoteDescription(data.userId, data.answer);
		}
	};

	const candidateManager = (data: { userId: string; candidate: RTCIceCandidateInit }) => {
	...
	useEffect(() => {
		socketManager.on("connect", () => {
			window.addEventListener("keydown", () => {
				users.forEach((userId) => {
					console.log(peerConnection.getConnection(userId)?.connection?.connectionState);
				});
			});
			getUserMedia(false);
